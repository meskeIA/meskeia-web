# ESTRATEGIA-EXPANSION.md - meskeIA Web

> **Creado**: 2026-03-30
> **Última actualización**: 2026-03-31
> **Estado**: Activo — documento vivo de planificación estratégica

---

## 1. Análisis Competitivo (Mercado Hispanohablante)

### 1.1 Posición de meskeIA

meskeIA es la **única plataforma en español** que combina todos estos ejes:

| Eje | meskeIA | Siguiente competidor |
|-----|---------|---------------------|
| N.º herramientas España-específicas | **354** | Calculord (~25) |
| Suites temáticas | **15** | CalcuWorld (~20 categorías genéricas) |
| Integración IA | **8 GPTs + MCP + llms.txt** | Calculord (1 asistente) |
| Accesibilidad dedicada | **15 apps** | Ninguno |
| Dark mode | **Sí** | Ninguno |
| Sin publicidad + sin registro | **Sí** | Calculord (sin ads), bancos (sin ads pero con registro) |
| Cursos estructurados | **13 cursos, 100+ capítulos** | Ninguno |
| Guías de decisión | **10 guías** | TaxDown (guías fiscales) |
| Datos fiscales centralizados | **data/fiscal/ con metadatos** | TaxDown, Calculord (parcial) |

### 1.2 Competidores por Tier

#### Tier 1 — Agregadores directos

| Competidor | URL | Tools | Fortaleza | Debilidad |
|------------|-----|-------|-----------|-----------|
| **CalcuWorld** | es.calcuworld.com | ~200-400 | SEO legacy, volumen | Genérico, sin España, lleno de ads |
| **PlanetCalc** | es.planetcalc.com | 566 | Mayor volumen | Académico, sin fiscal, ads agresivos |
| **MasterCalculadora** | mastercalculadora.com | ~200+ | Construcción/ingeniería | Sin fiscal España |
| **Calculord** | calculord.com | ~25 | Moderno, España, IA | Muy pequeño, solo laboral/fiscal |
| **CalculaAhora** | calculaahora.es | 11 | Alta calidad fiscal | Mínimo alcance, ads |

#### Tier 2 — Plataformas financieras

| Competidor | URL | Tools | Fortaleza | Debilidad |
|------------|-----|-------|-----------|-----------|
| **TaxDown** | taxdown.es | 18 | Marca fiscal premium | Solo impuestos, freemium |
| **Finect** | finect.com | 6 | Comunidad inversión | Solo 6 calculadoras |
| **Rankia** | rankia.com | Pocas | Mayor comunidad financiera ES | Casi sin calculadoras |
| **INEAF** | ineaf.es | 11 | Credibilidad institucional | Embudo de cursos |

#### Tier 3 — Simuladores bancarios

| Banco | Tools | Fortaleza | Debilidad |
|-------|-------|-----------|-----------|
| **BBVA** | ~15 | UX excelente, SEO imbatible | Embudo comercial, alcance limitado |
| **Santander** | ~8 | Marca confiable | Muy pocas herramientas |
| **Bankinter** | ~7 | Buen simulador de pensiones | Muy pocas herramientas |

#### Tier 4 — Mono-herramienta

CalcularIRPF.es, CalculadoraIRPF.es, CalculaFinanzas.es — una sola herramienta cada uno.

### 1.3 Conclusiones competitivas

1. **No existe competidor directo** que combine amplitud + especificidad España + gratuidad + IA
2. **Vigilar**: Calculord (si escala), TaxDown (domina SEO fiscal), bancos (SEO imbatible en hipotecas)
3. **Oportunidad**: Los nichos de alto impacto social están completamente vacíos

---

## 2. Nuevos Nichos de Expansión

### Priorización por impacto

| # | Nicho | Audiencia | Competencia | Impacto Social | Prioridad |
|---|-------|-----------|-------------|----------------|-----------|
| 1 | Cuidadores y Dependencia | Alta | **Cero** | Muy alto | 🔴 Alta |
| 2 | ~~Migración y Extranjería~~ | ~~Alta~~ | ~~Cero~~ | ~~Muy alto~~ | ❌ **DESCARTADO** |
| 3 | Salud Mental | Muy alta | Baja | Muy alto | 🔴 Alta |
| 4 | Crianza y Familia | Alta | Baja | Alto | ✅ **COMPLETADO** |
| 5 | Tercera Edad | Alta | **Cero** | Alto | ✅ **COMPLETADO** |
| 6 | Sostenibilidad y Hogar Eficiente | Alta | Baja | Alto | ✅ **COMPLETADO** |
| 7 | Derecho Cotidiano | Alta | Baja | Alto | 🟡 Media |
| 8 | Comunidades de Vecinos | Media | **Cero** | Medio | 🟢 Normal |
| 9 | Oposiciones y Empleo Público | Media | Baja | Medio | 🟢 Normal |
| 10 | Educación Financiera Juvenil | Media | Baja | Alto | 🟢 Normal |

---

### Nicho 1: Cuidadores y Dependencia

**Audiencia**: ~1,5M personas dependientes en España + millones de cuidadores informales
**Competencia**: Cero herramientas interactivas gratuitas
**Suite sugerida**: Salud y Bienestar (o nueva suite si crece)

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Orientador Grado de Dependencia | Baremo BVD oficial español para estimar grado | Media | ✅ Ya existía |
| Orientador Grado de Discapacidad | Test funcional RD 888/2022 | Media | ✅ Ya existía |
| Residencia vs Cuidado en Casa | Comparativa de 3 opciones de cuidado | Baja | ✅ Ya existía |
| Estimación de Prestaciones por Dependencia | Cuantías SAAD según grado + servicios + copago | Media | ✅ Implementada 2026-03-30 |
| Planificador de Turnos de Cuidadores | Calendario semanal visual con reparto equitativo | Baja | ✅ Implementada 2026-03-30 |
| Test de Zarit — Sobrecarga del Cuidador | Escala 22 ítems + recursos de ayuda | Baja | ✅ Implementada 2026-03-30 |
| Estimación de Deducción IRPF por Discapacidad | Mínimos personales y familiares IRPF | Media | ✅ Implementada 2026-03-30 |
| Checklist de Trámites de Dependencia | 6 fases, documentación, plazos orientativos | Baja | ✅ Implementada 2026-03-30 |

---

### Nicho 2: Migración y Extranjería en España

**Audiencia**: 6M+ extranjeros residentes
**Competencia**: Cero herramientas interactivas
**Suite sugerida**: Nueva suite "Migración y Trámites" o Legal y Fiscal

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Selector de tipo de visado/permiso | Según nacionalidad, motivo, duración | Media | ⬜ Por decidir |
| Calculadora de arraigo social | Requisitos, plazos, documentación | Media | ⬜ Por decidir |
| Simulador de puntos para nacionalidad | Años residencia, idioma, integración | Media | ⬜ Por decidir |
| Checklist de documentación por trámite | Según tipo de permiso/renovación | Baja | ⬜ Por decidir |
| Calculadora de homologación de títulos | Plazos estimados, tasas, requisitos | Baja | ⬜ Por decidir |
| Calculadora de tasas de extranjería | Modelo 790 según trámite | Baja | ⬜ Por decidir |
| Test de requisitos para reagrupación familiar | Ingresos mínimos, vivienda, documentación | Media | ⬜ Por decidir |

---

### Nicho 3: Salud Mental (Autoconocimiento)

**Audiencia**: Muy alta (demanda explosiva post-pandemia)
**Competencia**: Escalas clínicas públicas pero sin herramientas accesibles en español
**Suite sugerida**: Salud y Bienestar
**Disclaimer**: Nivel 2 ALTO obligatorio (orientativo, no sustituye profesional)

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Test PHQ-9 (depresión) | Escala validada + orientación y recursos | Baja | ⬜ Por decidir |
| Test GAD-7 (ansiedad) | Escala validada + recursos | Baja | ⬜ Por decidir |
| Test de calidad del sueño (Pittsburgh) | PSQI completo con puntuación | Media | ⬜ Por decidir |
| Test de estrés laboral (Maslach) | Burnout + agotamiento + despersonalización | Media | ⬜ Por decidir |
| Diario emocional visual | Registro diario con patrones visuales | Media | ⬜ Por decidir |
| Planificador de hábitos saludables | Seguimiento visual con racha | Media | ⬜ Por decidir |
| Test de bienestar general (WHO-5) | Escala OMS de bienestar subjetivo | Baja | ⬜ Por decidir |

---

### Nicho 4: Crianza y Familia

**Audiencia**: Alta (natalidad baja = alta demanda de información sobre ayudas)
**Competencia**: Baja
**Suite sugerida**: Salud y Bienestar + Finanzas

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Planificador Embarazo y Bebé | FPP, checklist trimestral, lista compras, vacunas | Media | ✅ Ya existía |
| Orientador Percentiles Infantiles | Peso y talla OMS bebés/niños | Media | ✅ Ya existía |
| Estimación de Prestación por Nacimiento | Cuantía SS, 16 semanas, requisitos por edad | Media | ✅ Implementada 2026-03-31 |
| Estimación de Baja Maternal y Paternal | Duración, distribución, extras (múltiple/prematuro) | Media | ✅ Implementada 2026-03-31 |
| Planificador de Gastos del Primer Año | 10 categorías, 3 niveles (ajustado/medio/confortable) | Baja | ✅ Implementada 2026-03-31 |
| ~~Selector de tipo de escolarización~~ | ~~Público/concertado/privado~~ | ~~Baja~~ | ❌ Descartado (sesgo ideológico) |
| Estimación de Deducción por Maternidad IRPF | 1.200€/año + 1.000€ guardería, Modelo 140 | Baja | ✅ Implementada 2026-03-31 |
| Test de Estilo Parental | Baumrind, 16 preguntas, 4 estilos, autoconocimiento | Baja | ✅ Implementada 2026-03-31 |

---

### Nicho 5: Tercera Edad / Envejecimiento Activo

**Audiencia**: Alta (población más envejecida de Europa)
**Competencia**: Cero
**Suite sugerida**: Jubilación y Patrimonio + Salud

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Estimador de Complemento a Mínimos | Pensión mínima según situación | Media | ✅ Implementada 2026-03-31 |
| ~~Simulador de pensión de viudedad~~ | ~~Requisitos + cuantía estimada~~ | ~~Media~~ | ✅ Ya existía (estimador-pension-viudedad) |
| ~~Test de fragilidad~~ | ~~FRAIL + orientación~~ | ~~Media~~ | ✅ Ya existía (test-fragilidad) |
| ~~Planificador de testamento orientativo~~ | ~~Guía interactiva~~ | ~~Alta~~ | ✅ Ya existía (orientacion-tramitacion-herencias) |
| Selector de Actividades según Movilidad | Recomendaciones personalizadas | Baja | ✅ Implementada 2026-03-31 |
| ~~Ayudas al alquiler mayores 65~~ | ~~Por CCAA~~ | ~~Media~~ | ❌ Descartada (alta variabilidad, difícil mantenimiento) |

---

### Nicho 6: Sostenibilidad y Hogar Eficiente

**Audiencia**: Alta (transición energética)
**Competencia**: Baja
**Suite sugerida**: Inmobiliaria y Hogar + Herramientas Técnicas

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Calculadora Huella de Carbono | Estimación anual por categorías | Media | ✅ Ya existía |
| Calculadora Gasto Energético | Consumo eléctrico electrodomésticos | Baja | ✅ Ya existía |
| Calculadora Eficiencia Energética | Ahorro aislamiento/ventanas/bomba calor | Media | ✅ Ya existía |
| Selector de Energía para el Hogar | Test calefacción: gas/aerotermia/biomasa/solar | Media | ✅ Ya existía |
| Simulador de Ahorro con Placas Solares | Producción, autoconsumo, amortización por zona | Alta | ✅ Implementada 2026-03-31 |
| Selector de Tarifa Eléctrica | PVPC vs. mercado libre: test 10 preguntas | Media | ✅ Implementada 2026-03-31 |
| Estimación de Ahorro Hídrico | 10 hábitos, litros y euros ahorrados | Baja | ✅ Implementada 2026-03-31 |
| Simulador de Subvenciones Rehabilitación | NextGen EU + deducciones IRPF | Alta | ✅ Implementada 2026-03-31 |
| Estimación de Certificación Energética | Letra orientativa (A-G), 8 factores | Media | ✅ Implementada 2026-03-31 |

---

### Nicho 7: Derecho Cotidiano

**Audiencia**: Alta
**Competencia**: Baja
**Suite sugerida**: Legal y Fiscal
**Disclaimer**: Nivel 1 CRÍTICO obligatorio

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Calculadora de plazos de prescripción | Por tipo de acción legal | Media | ⬜ Por decidir |
| Selector de tipo de reclamación | Guía según situación | Baja | ⬜ Por decidir |
| Simulador de costas judiciales | Estimación por cuantía y tipo | Media | ⬜ Por decidir |
| Calculadora de indemnización accidente | Baremo de tráfico vigente | Alta | ⬜ Por decidir |
| Checklist de derechos del consumidor | Por tipo de compra/servicio | Baja | ⬜ Por decidir |
| Calculadora de pensión compensatoria | Divorcio: estimación orientativa | Alta | ⬜ Por decidir |

---

### Nicho 8: Comunidades de Vecinos

**Audiencia**: Media (millones de comunidades en España)
**Competencia**: Cero
**Suite sugerida**: Inmobiliaria y Hogar

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Calculadora de derramas | Reparto por coeficiente de participación | Baja | ⬜ Por decidir |
| Simulador de quórums LPH | Votos necesarios según tipo de acuerdo | Media | ⬜ Por decidir |
| Presupuesto anual de comunidad | Plantilla interactiva por partidas | Media | ⬜ Por decidir |
| Calculadora de cuota comunitaria | Distribución justa según coeficientes | Baja | ⬜ Por decidir |

---

### Nicho 9: Oposiciones y Empleo Público

**Audiencia**: Media (~500.000 opositores activos en España)
**Competencia**: Baja
**Suite sugerida**: Estudiantes + Productividad

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Simulador de baremos | Puntuación por experiencia + formación + examen | Media | ⬜ Por decidir |
| Planificador de estudio por temario | Distribuir temas en semanas | Baja | ⬜ Por decidir |
| Calculadora de nota de corte estimada | Histórico + previsión | Media | ⬜ Por decidir |
| Selector de oposición por perfil | Test según estudios, preferencias, edad | Baja | ⬜ Por decidir |

---

### Nicho 10: Educación Financiera Juvenil

**Audiencia**: Media
**Competencia**: Baja
**Suite sugerida**: Finanzas + Estudiantes

#### Apps propuestas

| App | Descripción | Complejidad | Estado |
|-----|-------------|-------------|--------|
| Simulador de paga + ahorro | Gestionar dinero semanal/mensual | Baja | ⬜ Por decidir |
| Juego de presupuesto mensual | Gamificado con escenarios reales | Media | ⬜ Por decidir |
| Calculadora "¿cuánto tardo en ahorrar?" | Objetivo + ahorro mensual = plazo | Baja | ⬜ Por decidir |
| Quiz de conceptos financieros | Interés, inflación, ahorro, inversión | Baja | ⬜ Por decidir |

---

## 3. Registro de Progreso

### Apps implementadas desde este plan

| Fecha | Nicho | App | Estado |
|-------|-------|-----|--------|
| 2026-03-30 | Cuidadores | Estimación de Prestaciones por Dependencia | ✅ Build OK |
| 2026-03-30 | Cuidadores | Planificador de Turnos de Cuidadores | ✅ Build OK |
| 2026-03-30 | Cuidadores | Test de Zarit — Sobrecarga del Cuidador | ✅ Build OK |
| 2026-03-30 | Cuidadores | Estimación de Deducción IRPF por Discapacidad | ✅ Build OK |
| 2026-03-30 | Cuidadores | Checklist de Trámites de Dependencia | ✅ Build OK |
| 2026-03-31 | Crianza | Estimación de Prestación por Nacimiento | ✅ Build OK |
| 2026-03-31 | Crianza | Estimación de Baja Maternal y Paternal | ✅ Build OK |
| 2026-03-31 | Crianza | Planificador de Gastos del Primer Año del Bebé | ✅ Build OK |
| 2026-03-31 | Crianza | Estimación de Deducción por Maternidad IRPF | ✅ Build OK |
| 2026-03-31 | Crianza | Test de Estilo Parental | ✅ Build OK |
| 2026-03-31 | Sostenibilidad | Simulador de Ahorro con Placas Solares | ✅ Build OK |
| 2026-03-31 | Sostenibilidad | Selector de Tarifa Eléctrica | ✅ Build OK |
| 2026-03-31 | Sostenibilidad | Estimación de Ahorro Hídrico | ✅ Build OK |
| 2026-03-31 | Sostenibilidad | Simulador de Subvenciones Rehabilitación | ✅ Build OK |
| 2026-03-31 | Sostenibilidad | Estimación de Certificación Energética | ✅ Build OK |
| 2026-03-31 | Tercera Edad | Estimador de Complemento a Mínimos | ✅ Build OK |
| 2026-03-31 | Tercera Edad | Selector de Actividades según Movilidad | ✅ Build OK |

### Decisiones tomadas

| Fecha | Decisión |
|-------|----------|
| 2026-03-30 | Documento estratégico creado. Se revisará nicho por nicho antes de implementar. |
| 2026-03-30 | Nicho 1 (Cuidadores) completado: 5 apps nuevas + 3 existentes = 8 apps totales. Módulo data/fiscal/dependencia.ts creado. |
| 2026-03-30 | Nomenclatura: usar "Estimación" en lugar de "Calculadora" para reducir riesgo legal. |
| 2026-03-31 | Nicho 2 (Migración) **DESCARTADO**: riesgo legal inaceptable (consecuencias irreversibles, colectivo vulnerable, complejidad normativa inmanejable). |
| 2026-03-31 | Nicho 4 (Crianza) completado: 5 apps nuevas + 2 existentes = 7 apps totales. Módulo data/fiscal/maternidad.ts creado. Selector escolarización descartado (sesgo ideológico). |
| 2026-03-31 | Nicho 3 (Salud Mental) aplazado al final por decisión del usuario. |
| 2026-03-31 | Nicho 6 (Sostenibilidad) completado: 5 apps nuevas + 4 existentes = 9 apps totales. |
| 2026-03-31 | Nicho 5 (Tercera Edad) completado: 2 apps nuevas + 3 existentes (viudedad, fragilidad, herencias) + 1 descartada (ayudas alquiler). Datos pensiones mínimas añadidos a data/fiscal/pensiones.ts. |

---

## 4. Métricas de Éxito

- **Objetivo**: Cubrir al menos 5 nichos nuevos con 3+ apps cada uno
- **Medición**: Apps implementadas + analytics de uso (Turso)
- **Revisión**: Actualizar este documento tras cada sesión de desarrollo

---

**Última actualización**: 2026-03-31
**Autor**: Claude Code + Jace
**Proyecto**: meskeIA Web
