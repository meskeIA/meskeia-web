import { NextRequest, NextResponse } from 'next/server';
import { calcularRegla180 } from '@/lib/calculadoras/videografia';
import { getTursoClient, initializeDatabase } from '@/lib/turso';

const ALLOWED_ORIGINS = ['https://chat.openai.com', 'https://chatgpt.com'];

function corsHeaders(origin: string | null) {
  const allowed = origin && ALLOWED_ORIGINS.includes(origin) ? origin : ALLOWED_ORIGINS[1];
  return { 'Access-Control-Allow-Origin': allowed, 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type' };
}

export async function OPTIONS(req: NextRequest) {
  return new NextResponse(null, { status: 204, headers: corsHeaders(req.headers.get('origin')) });
}

export async function POST(req: NextRequest) {
  const origin = req.headers.get('origin');
  try {
    const body = await req.json();
    const fps: number = body.fps;
    if (!fps) return NextResponse.json({ error: 'Falta el parámetro fps.' }, { status: 400, headers: corsHeaders(origin) });

    const r = calcularRegla180(fps);

    try {
      await initializeDatabase();
      const db = getTursoClient();
      await db.execute({ sql: `INSERT INTO analytics (app_name, created_at, mode) VALUES (?, ?, ?)`, args: ['calculadora-regla-180-video', new Date().toISOString(), 'chatgpt'] });
    } catch { /* analytics no crítico */ }

    return NextResponse.json({
      fps,
      obturador_recomendado: r.obturadorFormateado,
      descripcion: r.descripcion,
      tabla_fps_comunes: r.fps_alternativos.slice(0, 8).map(f => ({ fps: f.fps, obturador: f.obturador })),
    }, { headers: corsHeaders(origin) });
  } catch {
    return NextResponse.json({ error: 'Error interno.' }, { status: 500, headers: corsHeaders(origin) });
  }
}
