 ...
 *
 * Toda la lógica del árbol vive en `./motor.ts` y toda la corrección en `./casos.ts`, que es
 * el mismo motor con el que el panel de arriba dibuja el árbol: aquí no se calcula NADA,
 * solo se pinta y se llama a `comprobarRespuesta`.
 */
...
export default function CasosAula() {
  ...
  function comprobar() {
    // parseSpanishNumber admite «3» y «-2»; devuelve NaN con cualquier otra cosa, y de ese
    // NaN se encarga comprobarRespuesta con un mensaje propio (nunca «NaN» en pantalla).
    ...
    <div className={styles.casosSection}>
      <div className={styles.casosHeader}>
        ...
        <p className={styles.casosIntro}>
          ...
          comprobar cada resultado insertando la secuencia en el simulador de arriba, con el
          modo (BST o AVL) que diga el enunciado.
        </p>
      </div>
      ...
      <div className={styles.casoCuerpo}>
        ...
        */}
        ...
        <p className={styles.casoModo}>
          <span aria-hidden="true">🌳</span> Modo{' '}
          <strong>{datos.modo === 'avl' ? 'AVL (con rebalanceo)' : 'BST (sin rebalanceo)'}</strong>{' '}
          · inserciones: {listaDeValores(datos.inserciones)}
          {datos.eliminaciones && datos.eliminaciones.length > 0
            ? ` · borrados: ${listaDeValores(datos.eliminaciones)}`
            : ''}
        </p>
        ...
        {verSolucion && (
          <div className={styles.casoSolucion}>
            ...
            {/* El árbol final por niveles: es la forma más corta de que el alumno compare su
                dibujo con el del simulador sin tener que teclear la secuencia otra vez. */}
            <p className={styles.casoArbolFinal}>
              Árbol final por niveles: {listaDeValores(recorridosDe(datos).niveles)}
            </p>
            {/* La unidad la pone `textoRespuesta`, no un recorte hecho aquí: un número suelto
                junto a media etiqueta se lee mal (hallazgo 830 de simulador-genetica). */}
