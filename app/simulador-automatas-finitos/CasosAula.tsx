 ...
 *
 * Toda la teoría de autómatas vive en `./casos.ts`, que a su vez la importa del motor con
 * el que la app de arriba determiniza, minimiza y recorre cadenas: aquí no se calcula
...
export default function CasosAula() {
  ...
  function comprobar() {
    // parseSpanishNumber admite «4» y también «4,0»; devuelve NaN con cualquier otra cosa,
    // y de ese NaN se encarga comprobarRespuesta con un mensaje propio (nunca «NaN» en
    // pantalla).
    ...
    <div className={styles.casosSection}>
      <div className={styles.casosHeader}>
        ...
        <p className={styles.casosIntro}>
          {TOTAL_CASOS} problemas de teoría de autómatas con solución, siempre los mismos y en
          el mismo orden. Un profesor puede decir «resuelve los casos 3, 7 y 11» y corregir sin
          ambigüedad. Puedes comprobar cada resultado dibujando el autómata en el editor de
          arriba y pulsando Determinizar o Minimizar.
        </p>
      </div>
      ...
      <div className={styles.casoCuerpo}>
        <h3 className={styles.casoTitulo}>
          ...
          {!practica && (
            <span className={styles.casoEtiquetaTipo}>
              {caso.categoria === 'abstracto' ? 'teoría' : 'aplicado'}
            </span>
          )}
        </h3>
        {/*
        ...
        */}
        ...
        {verSolucion && (
          <div className={styles.casoSolucion}>
            ...
            {/* Lo que se cuenta lo pone `textoRespuesta`, no un recorte hecho aquí: un «4»
                suelto no distingue 4 estados de 4 cadenas (hallazgo 830, simulador-genetica). */}
