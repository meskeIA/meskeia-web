 ...
 *
 * Toda la aritmética de stops vive en `./casos.ts`, que exporta las MISMAS funciones con las
 * que el medidor de exposición de la app calcula su ΔEV: aquí no se calcula NADA, solo se
...
export default function CasosAula() {
  ...
  const pregunta = practica ? practica.datos.pregunta : caso.datos.pregunta;
  const decimales = (practica ? practica.datos.decimales : caso.datos.decimales) ?? 2;
      ...
      <div className={styles.casoCuerpo}>
        ...
        <div className={styles.casoRespuesta}>
          ...
          <div className={styles.casoFila}>
            {/* inputMode="text" y NO "decimal": el teclado decimal de iOS no ofrece el signo
                menos, y aquí la mitad de las respuestas son negativas (menos luz). */}
            <input
              id="casos-respuesta"
              type="text"
              inputMode="text"
        ...
        {verSolucion && (
          <div className={styles.casoSolucion}>
            ...
            <p className={styles.casoResultado}>
              Respuesta: <strong>{textoRespuesta(esperado, pregunta, decimales)}</strong>
            </p>
          </div>
        )}
      </div>
