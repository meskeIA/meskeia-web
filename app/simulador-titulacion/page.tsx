...
export default function SimuladorTitulacion() {
  ...
  const V_eq = useMemo(() => volumenEquivalencia(C_analito, V_analito, C_titulante), [C_analito, V_analito, C_titulante]);
  ...
  /**
   * «Cargar en el simulador» de los casos para clase: pone el tipo y los deslizadores del caso
   * y deja la bureta a 0, para que el alumno vierta él el volumen con «+ 1 mL» y «+ Gota» y
   * vea el pH que se le pregunta. Un parámetro que el caso no fija (el pKa en ácido fuerte) se
   * queda como estaba.
   */
  const cargarCaso = useCallback((c: ConfiguracionSimulador) => {
    setTipo(c.tipo);
    setVAnalito(c.V_analito);
    setCAnalito(c.C_analito);
    setCTitulante(c.C_titulante);
    if (c.pKa !== undefined) setPKa(c.pKa);
    if (c.pKb !== undefined) setPKb(c.pKb);
    setVTitulante(0);
    document
      .querySelector('svg[aria-label="Curva de titulación"]')
  ...
  return (
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <CasosAula onCargarEnSimulador={cargarCaso} />
