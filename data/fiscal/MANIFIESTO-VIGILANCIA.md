...
- **Vigilar**: RD de revalorización (enero: pensión máx/mín, complemento mínimos, cuantía brecha género); avance anual de la tabla de edad ordinaria y de la ventana del sistema dual (cambia CADA año hasta 2044); sentencias TS/TJUE sobre brecha de género; RDL de medidas de pensiones (frecuentes).
  ⚠️ `RECLAMACION_PREVIA_SS` **no sigue el ciclo anual de pensiones**: es procedimiento, no cuantía, así que no lo mueve ningún RD de revalorización — solo una reforma de la LRJS. No re-sellarlo por rutina en enero: su sello dice de qué depende. Vive aquí porque las apps que lo usan son de pensiones, no porque lo sea.
...
- **Alerta (b) MATERIALIZADA — Madrid, 2026-08-12**: la Ley 3/2026, de 30 de junio, de Apoyo a la Empresa Familiar (BOE-A-2026-16019, en vigor desde el 01/07/2026) modifica los arts. 21 y 22 del Decreto Legislativo 1/2010 e introduce una **reducción del 99% EN BASE** por transmisión de empresa individual, negocio profesional o participaciones, extendida a Grupos I, II y III y a colaterales de cuarto grado, con permanencia de 5 años y participación mínima del 5% individual / 20% del grupo familiar. El módulo modela Madrid **solo como bonificación en cuota por parentesco** y no contiene ninguna reducción de empresa familiar (tampoco la estatal del 95% del art. 20.2.c LISD). Aplicado hoy como advertencia en `notas`; **modelarlo exige tocar el código** de `estimador-impuesto-sucesiones` y `simulador-heredar-vivienda`, porque cambia el orden de las operaciones. **CERRADA el 25/09/2026 sin modelar**: la empresa familiar queda fuera del ISD por criterio (§5.bis), así que la advertencia en `notas` es el estado final, no un paso intermedio.
...
- **Alerta (b) MATERIALIZADA — Madrid, 2026-08-12**: la misma Ley 3/2026 extiende la reducción del 99% en base a las **donaciones** de empresa familiar, con los requisitos de formalización que ya contempla el campo `requiereEscritura`. Ver la ficha de `sucesiones.ts`. App afectada: `estimador-impuesto-donaciones`. **CERRADA el 25/09/2026 sin modelar**, por el mismo descarte (§5.bis).
...
- **Vigilar**: coeficientes IIVTNU en cada LPGE; tipos ITP/AJD en leyes de medidas CCAA; tramos del ahorro en LPGE (precedente: nuevo tramo 30% > 300.000 € en 2023); nueva jurisprudencia constitucional sobre IIVTNU. **Y los beneficios autonómicos por colectivo, que son el punto débil demostrado del módulo**, por dos vías distintas: (a) la EDAD — el 19/08/2026 se verificaron tres comunidades en fuente oficial y las tres estaban desactualizadas en algún sitio: Murcia ≤40 (no <35, y sin límite de valor), La Rioja <40 al 4% (no <36 al 5%), Cataluña ≤35 desde el 27/06/2025 (no ≤32); (b) la NATURALEZA del beneficio — el 11/09/2026 Aragón resultó no tener tipos reducidos en absoluto, sino bonificaciones en cuota (12,5% del art. 121-4, 50%/60% de los arts. 121-5 y 160-3), y su tipo general era una escala de cinco tramos declarada como dos. Las comunidades cuya ficha sigue diciendo «<36» o «<35», y aquellas cuyo beneficio se haya anotado como tipo reducido sin abrir la norma, no se han contrastado: son material de la inmersión de enero.
...
- **Verificado**: 2026-06-17 (principal) · 2025-01-15 (IIVTNU). ⚠️ Bloque IIVTNU con sello antiguo: los coeficientes 2026 deben confirmarse. 11/09/2026: Aragón verificado entero contra el consolidado del BOE (`BOA-d-2005-90006`) y corregido, pero **el sello del módulo NO se mueve** — verificar una comunidad no autoriza a afirmar las 17 (PASO 4 de `/triaje-fiscal`).
...
| **Regímenes fiscales territoriales en IRPF e IS** (Ceuta y Melilla, La Palma) | 2026-09-11 | Levantado por dos RDL de la misma ventana: el **RDL 22/2026** (Ceuta) sube la bonificación del art. 33 LIS del 50 % al 60 %, crea una DA 64.ª en la Ley 35/2006 y toca cuotas de autónomos; el **RDL 23/2026** extiende a La Palma la deducción de IRPF análoga a la de Ceuta/Melilla (DA 57.ª). **No entra**: ambos son regímenes temporales (ejercicio 2026, o mientras dure la crisis migratoria de julio de 2026) sobre poblaciones de ~85.000 habitantes, y no hay ninguna cifra que invalidar porque `irpf.ts`, `sociedades.ts`, `modulos-irpf.ts` y `autonomos.ts` **nunca han modelado especialidades territoriales** — el hueco es de cobertura, no de vigilancia. ⚠️ **El descarte es sobre IRPF e IS, NO sobre el territorio**: `data/itp-ccaa.ts` sí trata Ceuta y Melilla en compraventa, con la bonificación del 50 % del art. 57 bis TRLITPAJD y los tres territorios donde no se liquida IVA, y eso se mantiene. **Se revisa si** el régimen deja de ser temporal (una LPGE que lo consolide) o si nace una app que calcule IRPF o IS por territorio: en ese caso el orden correcto es cubrir primero el caso general de Ceuta y Melilla, no La Palma. |
| **Reducciones por empresa familiar en el ISD** (la estatal del art. 20.2.c y 20.6 LISD y las de cada CCAA; la que lo levantó fue la del 99 % en base de Madrid, Ley 3/2026, BOE-A-2026-16019) | 2026-09-25 | **No se puede calcular con honestidad**: la reducción recae sobre el valor de la empresa, y valorar una empresa no cotizada o unas participaciones no es un dato que el usuario pueda teclear; además los requisitos (actividad económica real, porcentaje de participación, funciones de dirección, permanencia posterior) solo los comprueba un asesor con la documentación del negocio. Ninguna app ni tool del MCP la ha calculado nunca: `estimador-impuesto-sucesiones` y `estimador-impuesto-donaciones` la listan en su aviso visible como NO incluida, y las tools `consulta_herencia`, `calcular_sucesiones` y `calcular_donaciones` lo declaran en su descripción desde el 25/09/2026 (antes callaban). Ese día se retiró además del bloque educativo de donaciones un ejemplo que daba una cuota de «≈ 750 €» con la reducción aplicada. **Un cambio de cifras o requisitos de estas reducciones, estatal o autonómico, no se escala**: basta con que la nota de la CCAA en `sucesiones.ts`/`donaciones.ts` siga diciendo que la estimación no la aplica. **Se revisa si** el catálogo llega a tener una app de valoración de empresas cuyo resultado pueda alimentar el ISD. |
