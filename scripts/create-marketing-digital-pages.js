/**
 * Script para crear las páginas de capítulos del Curso de Marketing Digital
...
const contentPath = path.join(__dirname, 'curso-marketing-digital-content.json');
const content = JSON.parse(fs.readFileSync(contentPath, 'utf8'));

// Directorio base del curso
const courseDir = path.join(__dirname, '..', 'app', 'curso-marketing-digital');
...
function generateChapterPage(moduleId, chapterId, chapterData, moduleTitle) {
  ...
  const { introduction, sections, keyIdeas, actionItems, reflectionQuestions, curiosity } = chapterContent;
...
${actionItems.map((action, idx) => `            <div className={styles.actionCard}>
...
${reflectionQuestions.map(question => `            <div className={styles.reflectionCard}>
...
// Crear las páginas
console.log('📚 Creando páginas del Curso de Marketing Digital...\n');
...
for (const [moduleId, moduleData] of Object.entries(content.modules)) {
  ...
  for (const [chapterId, chapterData] of Object.entries(moduleData.chapters)) {
    ...
    const pageContent = generateChapterPage(moduleId, chapterId, chapterData, moduleData.title);
...
if (pagesSkipped > 0) {
  console.log('💡 Ejecuta este script de nuevo cuando se complete la generación de contenido.\n');
}
