 ...
 *
 * EL MOTOR (app/selector-tipo-vivienda/page.tsx:38-145 y 266-289): cada opción SUMA puntos a
 * uno o varios de los cinco tipos (piso, casa, ático, estudio, compartido) y gana el máximo. No
 * hay opciones eliminatorias ni penalizaciones, y el empate lo deshace el orden de declaración
 * (`val > puntos[max]` estricto, empezando por 'piso'): piso > casa > ático > estudio > compartido.
 *
 ...
 * «Casa Unifamiliar».
 *
 * HALLAZGOS ABIERTOS: al final, marcados con `test.fail()`. Afirman lo que DEBERÍA pasar, así que
...
// ─────────────────────────────────────────────────────────────────────────────
// HALLAZGOS ABIERTOS (Inspector, 25/09/2026)
// ─────────────────────────────────────────────────────────────────────────────
...
for (const [etiqueta, perfil] of [
  ...
  test.fail(`${etiqueta}: el resultado no es «Piso Compartido» ni «Estudio», que la propia app descarta para familias`, async ({ page }) => {
    await abrir(page);
...
test.fail('con presupuesto «Muy ajustado» no recomienda el ático sin decir nada del presupuesto', async ({ page }) => {
  await abrir(page);
...
test.fail('un empate exacto piso-casa se dice, o no se resuelve contra el jardín y la privacidad pedidos', async ({ page }) => {
  await abrir(page);
...
test.fail('la guía no publica gastos de compra ni tipos de ITP y AJD escritos a mano', async ({ page }) => {
  await abrir(page);
...
test.fail('los tipos normativos de la guía llevan su sello de fuente y fecha (DataReference)', async ({ page }) => {
  await abrir(page);
...
test.fail('el foco no cae a <body> al avanzar, y al terminar va al resultado', async ({ page }) => {
  await abrir(page);
...
test.fail('las opciones son radios: role="radio" con aria-checked, sin aria-pressed, una sola marcada', async ({ page }) => {
  await abrir(page);
...
test.fail('la barra de progreso anuncia la misma fracción que pinta', async ({ page }) => {
  await abrir(page);
  await opciones(page).nth(0).click();
  await botonAvanzar(page).click();
  await expect(page.getByText('Pregunta 2 de 10', { exact: true })).toBeVisible();
...
for (const tema of ['light', 'dark'] as const) {
  test.fail(`contraste de 4,5:1 en los textos del test y del resultado (tema ${tema})`, async ({ page }) => {
    await abrir(page);
...
test.fail('el JSON-LD WebApplication trae 4-8 características y la descripción entera', async ({ page }) => {
  await abrir(page);
...
test.fail('los porcentajes de la guía y de la FAQ llevan espacio duro antes del %', async ({ page }) => {
  await abrir(page);
  const guia = await abrirGuia(page);
...
test.fail('el resultado y la guía dan la misma superficie típica del estudio', async ({ page }) => {
  await abrir(page);
  await responder(page, SOLO_ESTUDIO);
...
test.fail('las ventajas y cautelas de la casa no dan por hecho que se compra', async ({ page }) => {
  await abrir(page);
  await responder(page, NORMAL_CASA);
  await expect(tituloResultado(page)).toHaveText('Tu tipo de vivienda ideal: Casa Unifamiliar');
...
test.fail('«Próximos pasos» manda los gastos de adquisición a la app que los calcula', async ({ page }) => {
  await abrir(page);
  await responder(page, NORMAL_PISO);
...
test.fail('la pregunta 6 no dice «soy poco tiempo en casa»', async ({ page }) => {
  await abrir(page);
  for (let i = 0; i < 5; i++) {
    await opciones(page).nth(0).click();
    await botonAvanzar(page).click();
